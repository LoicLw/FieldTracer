...
public class ShareActivity extends Activity {
	...
	private File appPath = new File(Environment.getExternalStorageDirectory(),
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (settings.getString("AutomatedTracesSharing", "").toString()
				.equalsIgnoreCase("True")) {
			ToggleButton traceToggle = (ToggleButton) findViewById(R.id.toggleButton1);
			traceToggle.setChecked(true);
		}

		if (settings.getString("AutomatedPOISharing", "").toString()
				.equalsIgnoreCase("True")) {
		...
		case DIALOG_LOAD_FILE:
			...
			builder.setItems(mFileList, new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					...
					ServalRhizomeTools.addFile(getBaseContext(), appPath + "/"
							+ mChosenFile);
					Builder dialog_ok = new AlertDialog.Builder(context);
					dialog_ok.setMessage("File added to Rhizome store");
					dialog_ok.setPositiveButton("OK", null);
					dialog_ok.show();
